...
function summary() {
  return configured().map(id => {
    ...
    return {
      ...
      // Maya: whether any store can be settled directly (PayFac) — the copy
      // says who receives the money, so it must be true.
      ...(id === 'maya' ? { payfac: Boolean(config.payfac) } : {})
    };
  });
}
...
async function forStore(storeId, call = post) {
  ...
  try {
    accepted = await call('store_payment_providers', {
      p_store: storeId, p_paypal_env: paypal.env(), p_maya_env: maya.env()
    }, null);
  } catch {
    // A database without 0015 yet: there, taking payments means PayPal (0011).
