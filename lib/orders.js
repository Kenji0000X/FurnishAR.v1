 ...
 * payment method (PayPal or Maya, lib/providers); every amount and payee
 * comes from begin_payment(), and every payment is re-read from its provider.
 *
 * The privileged acts — recording what PayPal or Maya did — need
...
/** Can this server take ANY online payment? (PayPal, Maya, or both.) */
...
/*
  0015's provider arguments are sent only for Maya. For PayPal the calls are
...
async function startPayment(ctx, orderId, providerId = 'paypal') {
  ...
  await serverRpc('server_record_payment_attempt', {
    ...
    ...providerArgs(provider.id, { p_reference: started.reference })
  }, ctx.token);
...
async function recordCompletedCapture(ctx, facts, attempt) {
  ...
  const recorded = await serverRpc('record_capture', {
    ...
    ...providerArgs(provider.id)
  }, ctx?.token || null);

  if (!recorded.duplicate) {
    if (!recorded.applied) {
      await announce(ctx, facts.orderId, 'unapplied', { provider: provider.id });
      // A Maya payment sits in FurnishAR's account: FurnishAR refunds it, not the shop.
      if (provider.id === 'maya') {
        await tellAdmins('payment_problem', {
          provider: 'maya', reference: attempt?.provider_reference || facts.captureId,
          detail: `A Maya payment of ${money2(facts.amount)} ${facts.currency} (payment ${facts.captureId}) could not be applied to its order. Refund it in Maya Manager.`
        }, ctx?.site);
      }
    }
    else {
...
const MAYA_REFERENCE = /^[A-Za-z0-9-]{6,64}$/;
const ATTEMPT_STATUS = { failed: 'DECLINED', none: 'CANCELLED' };

/**
 * Settles ONE Maya attempt from what Maya itself says, read with the secret
 * key. Shared by the buyer's return (ctx with their token) and the webhook
 * (ctx without). Never trusts the redirect or the webhook body: both only
 * name a reference.
 *
 *   paid      record_capture (idempotent on Maya's payment id), emails once
 *   mismatch  Maya took an amount the attempt did not ask for: recorded
 *             anyway (the money exists), which the database marks unapplied,
 *             so the order does not move and a refund is asked for
 *   pending   nothing recorded yet
 *   failed    the attempt is marked declined; `notifyFailure` emails the buyer
 *   none      no payment at Maya (the buyer cancelled or left)
 */
async function settleMaya(ctx, attempt, { notifyFailure = false } = {}) {
  const mayaProvider = providers.PROVIDERS.maya;
  const payments = await maya.paymentsForReference(attempt.provider_reference).catch(error => {
    // Maya answers 404 for a reference with no payment yet.
    if (error.status === 404) return [];
    throw fail(502, "We couldn't confirm the payment with Maya. Please try again.");
  });
  const verdict = mayaProvider.judgePayments(attempt, payments);
  ...
  if (verdict.state === 'mismatch') {
    await tellAdmins('payment_problem', {
      provider: 'maya', order_id: attempt.order_id, reference: attempt.provider_reference,
      detail: `Maya reports a successful payment in ${payment.currency || 'an unknown currency'}, not ${attempt.currency}. It was not recorded; refund it in Maya Manager.`
    }, ctx?.site);
  ...
  if (verdict.state === 'failed' && notifyFailure) await announce(ctx, attempt.order_id, 'payment_failed', { provider: 'maya' });
  return { state: verdict.state === 'failed' ? 'failed' : 'cancelled', orderId: attempt.order_id };
}

/** The buyer is back from Maya: what happened to their payment? */
async function verifyMaya(ctx, reference) {
  if (!providers.PROVIDERS.maya.ready()) throw fail(503, 'Maya payments are not switched on.', 'payments_unconfigured');
  if (!MAYA_REFERENCE.test(String(reference || ''))) throw fail(400, 'Unknown Maya payment.');
  const attempt = await serverRpc('server_payment_attempt_by_reference',
    { p_provider: 'maya', p_reference: reference }, ctx.token);
  if (!attempt) throw fail(404, 'Unknown Maya payment.');
  // Only the buyer who started it may ask about it (RLS: their own orders).
  if (!await ownsOrder(ctx, attempt.order_id)) throw fail(404, 'Unknown Maya payment.');
  const result = await settleMaya(ctx, attempt);
  return { provider: 'maya', orderId: result.orderId, state: result.state, status: result.status ?? null,
...
const ACTIONS = {
  ...
  /* The buyer's return from a provider that captures by itself (Maya). */
  async verify(ctx, body) {
    const provider = providers.get(body.provider);
    if (provider.id !== 'maya') throw fail(400, 'Unknown payment method.');
    return verifyMaya(ctx, body.reference);
  },
...
function billingConfig({ admin = false } = {}) {
  const config = paypal.validateConfig();
  const mayaConfig = maya.validateConfig();
  return {
    ...
    ...(admin ? {
      ...
      maya: {
        configured: mayaConfig.configured, sandbox: mayaConfig.sandbox, environment: mayaConfig.env,
        payfac: mayaConfig.payfac, payfacFeeMode: mayaConfig.payfacFeeMode,
        problems: mayaConfig.problems, warnings: mayaConfig.warnings
      }
    } : {})
  };
}
...
module.exports = {
  ...
  feeModeFor, matchesAttempt, recordCompletedCapture, settleMaya, announce, tellAdmins, fail
};
