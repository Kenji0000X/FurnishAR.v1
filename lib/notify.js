...
function render({ heading, lines = [], rows = [], action }) {
  ...
  return {
    html: `<div style="font-family:system-ui,sans-serif;font-size:15px;line-height:1.5;color:#1b2426;max-width:560px">
      ...
      <p style="margin:24px 0 0;font-size:12px;color:#5b6769">FurnishAR · Payments are made on PayPal's or Maya's own page; FurnishAR never sees your password or card.</p></div>`,
...
const PROVIDER_NAME = { paypal: 'PayPal', maya: 'Maya' };
...
/**
 * Where the money is, in words the shop can rely on (0015). PayPal pays the
 * shop's own account. Maya with platform collect pays FurnishAR's account,
 * which owes the shop its share. Maya PayFac settles to the shop's
 * sub-merchant.
 */
function whereTheMoneyIs(c) {
  ...
  if (p.fee_mode === 'platform_collect') {
    return `Payment was confirmed through Maya. FurnishAR's Maya account received ${peso(p.amount)}; `
      + `your share of ${peso(Number(p.amount) - Number(p.platform_fee || 0))} is owed to you and paid out by FurnishAR.`;
  }
  return `Payment was confirmed through Maya and is settled by Maya to your store's Maya account; FurnishAR's 10% service fee on this order is ${fee}.`;
}
  ...
  switch (event) {
      ...
      return [
        ...
        { to: c.store_emails, subject: `${ref}: deposit paid — start the build`, heading: 'Deposit paid',
          lines: [`${c.buyer_name} paid the ${peso(c.amount_paid)} deposit for ${c.product_name}.`,
                  ...((latestPayment(c)?.provider || 'paypal') === 'maya' ? [whereTheMoneyIs(c)] : []), ...spec,
    ...
    case 'unapplied': {
      ...
      return [{ to: c.store_emails, subject: `${ref}: payment needs a refund`, heading: 'A payment could not be applied',
        lines: [`A ${providerName(provider)} payment for ${c.product_name} from ${c.buyer_name} (${c.buyer_email}) arrived after the order had moved on (paid twice, or the stock ran out).`,
                provider === 'maya'
                  ? 'FurnishAR received it through Maya and will refund the buyer; you do not need to do anything.'
    ...
    case 'payment_failed':
      return [{ to: c.buyer_email, subject: `${ref}: payment not completed`, heading: 'Your payment did not go through',
        lines: [`${providerName(c.provider)} did not complete your payment for ${itemLine(c)}. You have not been charged.`,
    ...
    case 'refund_completed': {
      ...
      return [
        { to: c.buyer_email, subject: `${ref}: refund of ${amount}`, heading: 'Your refund is on its way',
          lines: [`${c.store_name} refunded ${amount} for ${itemLine(c)} through ${providerName(c.provider)}.`,
                  `${providerName(c.provider)} returns it to the account or card you paid with; banks can take a few days to show it.`],
          action: receipt },
...
function accountMessages(event, c, site) {
  ...
  switch (event) {
    case 'buyer_welcome':
      return [{ to: c.email, subject: 'Welcome to FurnishAR', heading: `Welcome, ${c.full_name || 'there'}`,
        lines: ['Your shopper account is ready. Measure your room, stand furniture in it at true size, and buy from shops in Occidental Mindoro.',
                'You do not need a payment account with us: you pay on PayPal\'s or Maya\'s own page, only when you check out.'],
    ...
    case 'platform_fee_recorded': {
      ...
      const sandbox = c.sandbox ? [`This is the ${via} sandbox: no real money moves.`] : [];
      const text = {
        ...
        platform_collect: ['Maya payment received by FurnishAR',
          `FurnishAR's Maya account received a ${peso(c.amount)} payment (${c.stage}). ${peso(c.platform_fee)} is FurnishAR's fee; `
          + `${peso(Number(c.amount) - Number(c.platform_fee))} is owed to the shop and appears in Billing until paid out.`],
        provider_settlement: ['Platform fee expected via Maya settlement',
          `A ${peso(c.amount)} payment (${c.stage}) settled to the shop's Maya sub-merchant. ${peso(c.platform_fee)} is expected via Maya's settlement; it is not collected until reconciled against Maya's report.`],
