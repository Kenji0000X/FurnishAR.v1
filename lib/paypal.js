...
function validateConfig() {
  ...
  // How a shop connects its PayPal account:
  //   merchant_id        (default) the owner enters their Merchant ID and PayPal
  //                      checks it with a test order. Needs no partner approval.
  //   partner_referrals  PayPal's hosted seller onboarding. Needs PayPal to
  //                      enable Partner Referrals for this app.
  const rawSeller = env('PAYPAL_SELLER_ONBOARDING').toLowerCase();
  let sellerMode = rawSeller || 'merchant_id';
  if (!['merchant_id', 'partner_referrals'].includes(sellerMode)) {
    problems.push(`PAYPAL_SELLER_ONBOARDING is "${rawSeller}"; it must be "merchant_id" or "partner_referrals". Using merchant_id.`);
    sellerMode = 'merchant_id';
  }
  const onboarding = sellerMode === 'partner_referrals'
    ? Boolean(c.clientId && c.secret && c.partnerMerchantId)
    : Boolean(c.clientId && c.secret);
  if (sellerMode === 'partner_referrals' && !c.partnerMerchantId) {
    problems.push('PAYPAL_PARTNER_MERCHANT_ID is not set, so shops cannot connect through Partner Referrals.');
  }
...
/** A sum of PayPal amounts, in pesos, added in centavos (lib/money.js). Null for none. */
function sumAmounts(list) {
  if (!Array.isArray(list) || !list.length) return null;
  const centavos = list.reduce((sum, item) => sum + phpToCentavos(String(item?.amount?.value ?? '0')), 0);
  return Number(centavosToPhp(centavos));
}

/**
 * The platform fees PayPal reports taking FOR FURNISHAR: a fee paid to some
 * other payee is not FurnishAR's, so it never counts as collected.
 */
function platformFeesForUs(list) {
  if (!Array.isArray(list)) return null;
  const { partnerMerchantId } = credentials();
  const ours = list.filter(fee => !fee?.payee?.merchant_id || !partnerMerchantId || fee.payee.merchant_id === partnerMerchantId);
  return sumAmounts(ours);
}

/** A single PayPal money field, in pesos, or null when PayPal did not report it. */
function moneyField(field) {
  return field?.value == null ? null : Number(centavosToPhp(phpToCentavos(String(field.value))));
}
...
function summarise(order) {
  ...
  return {
    ...
    platformFeeCollected: platformFeesForUs(capture?.seller_receivable_breakdown?.platform_fees),
    // PayPal's own processing fee and what reaches the seller: PayPal's
    // figures, kept apart from FurnishAR's 10% (null when not reported).
    processingFee: moneyField(capture?.seller_receivable_breakdown?.paypal_fee),
    sellerNet: moneyField(capture?.seller_receivable_breakdown?.net_amount)
  };
}
...
/**
 * Can this Merchant ID receive a payment? Asked of PayPal itself: it creates
 * a ₱1.00 order payable to that merchant, which PayPal refuses for an id that
 * does not exist or cannot receive money. The order is never approved or
 * captured, so nothing is charged; PayPal discards it on its own.
 * Returns { ok: true } or { ok: false, issue }.
 */
const PAYEE_PROBLEMS = /PAYEE|MERCHANT|RECEIVER|ACCOUNT_(RESTRICTED|LOCKED|CLOSED)/;

async function verifyPayee(merchantId, fetchImpl) {
  try {
    await call('/v2/checkout/orders', {
      method: 'POST',
      requestId: `verify-${merchantId}-${Date.now()}`,
      fetchImpl,
      body: {
        intent: 'CAPTURE',
        purchase_units: [{
          description: 'FurnishAR account check (never charged)',
          amount: { currency_code: 'PHP', value: '1.00' },
    ...
    if (PAYEE_PROBLEMS.test(String(error.issue || ''))) {
      return { ok: false, issue: error.issue || 'PAYEE_ACCOUNT_INVALID' };
    }
    throw error;
  }
}
...
module.exports = {
  ...
  createPartnerReferral, findMerchantByTracking, merchantIntegration, sellerStatus, verifyPayee,
