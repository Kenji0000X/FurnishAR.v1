...
export function roomDimensions(surfaces, options = {}) {
  ...
  if (!floor) {
    return {
      ...
      perimeter: null, volume: null, wallSpans: [],
  ...
  */
  ...
  return {
    ...
    /*
       Perimeter and volume — the P and V a room scanner is expected to show.

       Perimeter comes from the RECTANGLE, so that the three numbers on screen
       agree with each other. The panel reports length and width; if P were
       summed from the raw floor polygon it would not equal 2(L+W), and
       somebody checking the arithmetic would find the scanner contradicting
       itself. (It is NOT that the polygon's perimeter is wildly larger —
       measured on a zigzag scan edge it comes out within a percent. The
       reason is consistency, not size.)

       Volume needs a height, so it stays null until one is measured rather
       than being computed from a typical ceiling.
    */
    perimeter: rectangle ? 2 * (rectangle.length + rectangle.width) : null,
    volume: rectangle && height !== null
      ? footprintArea(floor.polygon) * height
      : null,

    /* Each wall's own run and height, for the labels that sit on them in the
       camera view. Only the ones tall enough to be a wall rather than a
       tracking artefact. */
    wallSpans: walls
      .map(wall => {
        const ys = wall.polygon.map(point => point.y);
        let run = 0;
        for (let i = 0; i < wall.polygon.length; i++) {
          for (let j = i + 1; j < wall.polygon.length; j++) {
            const a = wall.polygon[i];
            const b = wall.polygon[j];
            run = Math.max(run, Math.hypot(a.x - b.x, a.z - b.z));
          }
        }
        return { run, height: Math.max(...ys) - Math.min(...ys) };
      })
      .filter(span => span.run > 0.3),
