...
/* ------------------------------------------------- drawing it on the view -- */

/*
   Rough lens angles for a phone's rear camera held in portrait.

   Nothing in the browser reports the real field of view — getUserMedia gives
   resolution, not intrinsics — so these are estimates, and that matters for
   exactly one thing: WHERE a marker is drawn on the picture. It does not
   touch any measurement. The distances and the room come out of
   d = h*tan(theta), which never consults these numbers.

   So a marker near the middle of the frame sits almost exactly on its real
   spot, and one near the edge can be out by a few degrees' worth of pixels.
   The reticle at dead centre is exact by construction.
*/
export const DEFAULT_FOV = { x: 60, y: 76 };

/**
 * Where on screen does a floor point appear, given where the phone is
 * pointing right now?
 *
 * This is what lets a tapped corner stay stuck to the floor while the phone
 * pans across the room, the way a measuring app's markers do. It is an
 * honest perspective projection of the floor plane and not world tracking:
 * it holds while the person turns and tilts on one spot, and breaks the
 * moment they WALK, because every point is stored relative to where they
 * were standing. Without ARCore there is nothing that can follow a walk, so
 * the interface has to ask them to stay put rather than pretend otherwise.
 *
 * Returns fractions of the viewport, 0..1 from the top-left, plus whether
 * the point is actually within the frame.
 */
export function projectFloorPoint({ point, eyeHeight, tiltDegrees, bearingDegrees, fov = DEFAULT_FOV }) {
  if (!point || !(eyeHeight > 0) || !Number.isFinite(tiltDegrees) || !Number.isFinite(bearingDegrees)) {
    return { u: null, v: null, visible: false, distance: null };
  }
  const distance = Math.hypot(point.x, point.z);
  // Bearing of the point from where you stand, in the same frame as the
  // compass readings the corners were taken with.
  const pointBearing = Math.atan2(point.x, -point.z) * 180 / Math.PI;
  // Its angle down from vertical — the same theta the distance came from.
  const pointTilt = Math.atan2(distance, eyeHeight) * 180 / Math.PI;

  const dYaw = bearingDelta(bearingDegrees, pointBearing);
  const dPitch = pointTilt - tiltDegrees;

  /* v is inverted: a point FURTHER away has a larger theta and appears
     HIGHER in the frame, while screen coordinates grow downward. */
  const u = 0.5 + dYaw / fov.x;
  const v = 0.5 - dPitch / fov.y;

  return {
    u, v, distance,
    visible: u >= -0.15 && u <= 1.15 && v >= -0.15 && v <= 1.15
  };
}
