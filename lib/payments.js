...
async function ownAccount(ctx, storeId, environment) {
  ...
  const response = await callSupabase(
    `${url}/rest/v1/store_payment_accounts?store_id=eq.${encodeURIComponent(storeId)}&environment=eq.${environment}&provider=eq.paypal`
...
/** A payee refusal from PayPal, in words a shop owner can act on. */
function payeeProblem(issue) {
  if (/RESTRICTED|LOCKED|CLOSED/.test(issue)) {
    return 'PayPal says this account cannot receive payments right now (it is restricted or closed). Log in to PayPal to check it.';
  }
  return 'PayPal does not recognise that Merchant ID. Check it in PayPal → Account Settings → Business information and paste it again.';
}

/** Is this merchant id a real PayPal account that can receive money? PayPal decides. */
async function checkMerchant(merchantId) {
  const result = await paypal.verifyPayee(merchantId).catch(error => {
    const why = error.code === 'payments_unavailable' ? ' (PayPal did not accept this site\'s Client ID / Secret)'
      : error.issue && error.issue !== 'UNKNOWN' ? ` (PayPal said: ${error.issue})` : '';
    throw fail(502, `We couldn't reach PayPal to check the account${why}. Please try again.`);
  });
  return result;
}

/** Records a merchant-id connection (or its loss) for a store. */
async function recordLink({ storeId, merchantId, status, detail, site, token = null }) {
  const config = paypal.validateConfig();
  const recorded = await serverRpc('server_record_payment_account', {
    p_env: config.env, p_store: storeId, p_tracking_id: null, p_merchant_id: merchantId,
    p_status: status, p_receivable: status === 'CONNECTED', p_email_confirmed: status === 'CONNECTED',
    p_partner_fee: false, p_detail: detail
  }, token);
  if (recorded?.found) await announceStatus(recorded.store_id, recorded.before, recorded.status, detail, site);
  return recorded;
}

function requireMerchantMode() {
  const config = paypal.validateConfig();
  if (config.sellerMode !== 'merchant_id') throw fail(409, 'Connect PayPal with the Connect PayPal button instead.');
  if (!config.onboarding || !serverSecretReady()) {
    throw fail(503, 'Connecting PayPal is not switched on for this site yet.', 'onboarding_unconfigured');
  }
  return config;
...
const STORE_ACTIONS = {
  /**
   * Link a store to a PayPal account by its Merchant ID (the default way:
   * needs no PayPal partner approval). The owner is checked by the database
   * first; then PayPal itself must accept the id as a payee; only then is
   * the store CONNECTED. A shop's PayPal account is where it is paid, never
   * who it is.
   */
  async link(ctx, body) {
    if (!UUID.test(String(body.storeId || ''))) throw fail(400, 'Unknown shop.');
    const merchantId = String(body.merchantId || '').trim().toUpperCase();
    if (!MERCHANT_ID.test(merchantId)) {
      throw fail(400, 'A PayPal Merchant ID is 8–20 letters and numbers, like 7XK2QJ9LMN4PA.');
    }
    const config = requireMerchantMode();
    // Membership, as this user, before anything goes to PayPal.
    await serverRpc('server_payment_onboarding_started', {
      p_store: body.storeId, p_env: config.env, p_tracking_id: `link-${body.storeId}-${crypto.randomBytes(6).toString('hex')}`
    }, ctx.token);
    const check = await checkMerchant(merchantId);
    if (!check.ok) {
      await recordLink({ storeId: body.storeId, merchantId: null, status: 'ERROR', detail: payeeProblem(check.issue), site: ctx.site, token: ctx.token });
      throw fail(400, payeeProblem(check.issue), 'payee_invalid');
    }
    const recorded = await recordLink({
      storeId: body.storeId, merchantId, status: 'CONNECTED',
      detail: 'Linked by Merchant ID and accepted by PayPal as a payee.', site: ctx.site, token: ctx.token
    });
    if (recorded?.conflict) throw fail(409, 'That PayPal account is already connected to another store.', 'merchant_in_use');
    return { status: 'CONNECTED', ready: true };
  },

  /** The owner disconnects their store's PayPal account. Checkout closes. */
  async unlink(ctx, body) {
    if (!UUID.test(String(body.storeId || ''))) throw fail(400, 'Unknown shop.');
    const config = paypal.validateConfig();
    await serverRpc('server_payment_onboarding_started', {
      p_store: body.storeId, p_env: config.env, p_tracking_id: `unlink-${body.storeId}-${crypto.randomBytes(6).toString('hex')}`
    }, ctx.token);
    await recordLink({ storeId: body.storeId, merchantId: null, status: 'NOT_CONNECTED', detail: 'Disconnected by the store owner.', site: ctx.site, token: ctx.token });
    return { status: 'NOT_CONNECTED', ready: false };
  },

  /** An admin disconnects a store's PayPal account (e.g. it looks wrong). */
  async 'admin-unlink'(ctx, body) {
    if (!UUID.test(String(body.storeId || ''))) throw fail(400, 'Unknown shop.');
    ...
    await recordLink({ storeId: body.storeId, merchantId: null, status: 'NOT_CONNECTED',
      detail: 'Disconnected by a FurnishAR administrator. Connect PayPal again, or contact FurnishAR.', site: ctx.site });
  ...
  async connect(ctx, body) {
    ...
    if (config.sellerMode !== 'partner_referrals') throw fail(409, 'Enter your PayPal Merchant ID to connect.');
    ...
    const referral = await paypal.createPartnerReferral({
      ...
      // PayPal's reason code (e.g. NOT_AUTHORIZED) is not a secret and is
      // what the person fixing the setup needs; the credentials never are.
      const why = error.code === 'payments_unavailable' ? 'PayPal did not accept this site\'s Client ID / Secret'
        : error.issue && error.issue !== 'UNKNOWN' ? `PayPal said: ${error.issue}` : null;
      throw fail(502, `PayPal couldn't start the connection${why ? ` (${why})` : ''}. Please try again in a moment.`);
    });
  ...
  async refresh(ctx, body) {
    ...
    if (config.sellerMode === 'merchant_id') {
      // Only an account that is (or was) connected is re-checked; a
      // disconnected or refused one needs its Merchant ID entered again.
      if (!own.merchant_id || !['CONNECTED', 'PAYMENTS_NEED_ATTENTION'].includes(own.onboarding_status)) {
        throw fail(404, 'Enter your PayPal Merchant ID first.');
      }
      const check = await checkMerchant(own.merchant_id);
      const status = check.ok ? 'CONNECTED' : 'PAYMENTS_NEED_ATTENTION';
      const detail = check.ok ? 'Linked by Merchant ID and accepted by PayPal as a payee.' : payeeProblem(check.issue);
      await recordLink({ storeId: body.storeId, merchantId: own.merchant_id, status, detail, site: ctx.site, token: ctx.token });
      return { status, detail, ready: check.ok };
    }
...
async function onCapture(event, site) {
  ...
  if (capture.captureStatus !== 'COMPLETED' || !capture.captureId) return 'capture not completed';
  if (!orders.matchesAttempt(capture, attempt)) {
    // Money PayPal says was captured, but not what FurnishAR asked for: never
    // applied, never announced as received. An admin looks at it (once: the
    // event is claimed once).
    await orders.tellAdmins('payment_problem', {
      provider: 'paypal', reference: capture.captureId,
      detail: `PayPal capture ${capture.captureId} (${capture.amount} ${capture.currency}) does not match the payment FurnishAR started for this order. It was not recorded; check it in PayPal and refund it if needed.`
    }, site);
...
async function onRefund(event, site) {
  ...
  if (recorded.completed) {
    ...
    // The superadmin learns what happened to FurnishAR's fee on this payment.
    try {
      const [contacts, admins] = await Promise.all([
        serverRpc('server_order_contacts', { p_order: recorded.order_id }),
        serverRpc('server_admin_emails', {})
      ]);
      if (contacts && admins?.length) {
        await send('paypal_refund_admin', { ...contacts, admin_emails: admins, refund_amount: recorded.amount,
          refund_platform_fee: recorded.platform_fee_refunded, fee_mode: recorded.fee_mode }, site);
      }
    } catch (error) {
      console.error(`[payments] refund admin email failed: ${error.message}`);
    }
  }
...
async function sendPaymentReminders(site) {
  ...
  // The same scheduled run retries payment emails that failed (0017).
  const notifications = await dispatchPaymentNotifications({ site, limit: 50 })
    .catch(error => ({ error: error.message }));
  return { due: due.length, sent, cooldownHours, maxCount, notifications };
}
